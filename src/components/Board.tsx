import { useState } from 'react';
import type { BoardSize, BoardState, Player } from '../game/logic';
import { GridLines, InkO, InkX, StrikeLine } from './decor';

interface BoardProps {
  board: BoardState;
  size: BoardSize;
  current: Player;
  /** может ли человек сейчас ставить отметку */
  interactive: boolean;
  ended: boolean;
  line: number[] | null;
  onTapCell: (i: number) => void;
}

export function BoardView({
  board,
  size,
  current,
  interactive,
  ended,
  line,
  onTapCell,
}: BoardProps) {
  const [hover, setHover] = useState<number | null>(null);
  const pad = size === 5 ? 'inset-[11%]' : 'inset-[13%]';

  return (
    <div className="relative aspect-square w-full">
      <GridLines size={size} className="absolute inset-0 h-full w-full text-[#8ba0c4]" />

      <div
        className="absolute inset-0 grid"
        style={{
          gridTemplateColumns: `repeat(${size}, minmax(0, 1fr))`,
          gridTemplateRows: `repeat(${size}, minmax(0, 1fr))`,
        }}
      >
        {board.map((value, i) => {
          const inWin = line?.includes(i) ?? false;
          const showGhost = !value && interactive && !ended && hover === i;
          const clickable = (!value && interactive) || ended;
          const row = Math.floor(i / size) + 1;
          const col = (i % size) + 1;
          return (
            <button
              key={i}
              type="button"
              onClick={() => onTapCell(i)}
              onMouseEnter={() => setHover(i)}
              onMouseLeave={() => setHover((h) => (h === i ? null : h))}
              aria-label={`Клетка: ряд ${row}, столбец ${col}${
                value ? (value === 'x' ? ', крестик' : ', нолик') : ', свободно'
              }`}
              className={`relative touch-manipulation rounded-xl transition-colors duration-150 ${
                clickable ? 'cursor-pointer' : 'cursor-default'
              } ${!value && interactive && !ended ? 'hover:bg-[#e9f0fb]/70' : ''}`}
            >
              {inWin && (
                <span className="anim-pop absolute inset-[6%] rounded-lg bg-hl/60" />
              )}
              {value === 'x' && <InkX className={`absolute ${pad} text-ink`} />}
              {value === 'o' && <InkO className={`absolute ${pad} text-pen`} />}
              {showGhost &&
                (current === 'x' ? (
                  <InkX still className={`absolute ${pad} text-ink opacity-25`} />
                ) : (
                  <InkO still className={`absolute ${pad} text-pen opacity-25`} />
                ))}
            </button>
          );
        })}
      </div>

      {line && (
        <StrikeLine
          line={line}
          size={size}
          className="pointer-events-none absolute inset-0 z-10 h-full w-full text-pen"
        />
      )}
    </div>
  );
}
